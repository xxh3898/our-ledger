...
## 10D-3B3C bridge와 10D-3B3F Manifested Runtime V2 source
...
- Compose subprocess는 fixed minimal environment와 external owner-only env만 사용해 ambient `DOCKER_HOST`, image 또는 secret override를 authority로 삼지 않는다. candidate cutover는 API/Web만 `--no-deps --pull never`로 바꾸고 PostgreSQL volume/network/container를 유지한다. 따라서 persistent PostgreSQL의 `config_files` label이 생성 당시 prior runtime release를 가리킬 수 있으며, 다음 predeploy backup은 같은 managed release store의 verified V1/V2 release와 current Compose bytes가 exact compatible한 경우에만 이를 허용한다. 단순 path equality 제거, arbitrary byte-equal external file과 PostgreSQL recreate는 해결책으로 사용하지 않는다.
