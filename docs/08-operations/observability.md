...
10D-2B2 deploy transaction은 같은 external reporter의 `deployments` mode만 재사용한다. event key 하나에 `RUNNING` 뒤 `SUCCESS`, `FAILED` 또는 `ROLLED_BACK` terminal lifecycle을 보내며 branch `main`, candidate/previous commit SHA, bounded actor/failure stage/summary와 rollback boolean만 포함한다. payload는 compact JSON stdin, `shell=False`, fixed timeout이고 reporter stdout/stderr는 폐기한다. HomeOps endpoint/secret/HMAC/spool, absolute host path, token, raw exception 또는 사용자·금융 data를 읽거나 전송하지 않는다. reporter validation은 transaction 시작 전 fail closed하고, 검증 뒤 reporter nonzero/timeout은 이미 결정된 application transaction 성공/rollback을 뒤집지 않는다. 이 source gate는 actual reporter를 호출하지 않고 fake reporter만 사용한다.
...
10D-3에서 exact production target, 첫 verified backup, external owner-only state/reporter/bootstrap, existing monitored-service exact HTTPS target와 rollback을 확인한 뒤 설치한다. Category나 memo 같은 사용자 행동 데이터와 raw health response를 외부 분석 서비스로 보내지 않는다.
