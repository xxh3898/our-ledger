...
public command는 fixed production app root의 B1 host worker를 통해 shared project operation lock을 먼저 얻고, pending recovery가 없을 때만 non-executable internal core를 호출한다. core의 Docker CLI authority는 ambient `PATH`나 environment override가 아니라 Darwin `/usr/local/bin/docker`, Linux `/usr/bin/docker` fixed absolute path다. `Darwin|Linux` 외 platform, missing/non-executable CLI 또는 Compose plugin unavailable은 Docker operation 전에 fail closed하며 Compose와 inspect는 선택한 같은 absolute executable만 사용한다. 이 authority는 Mac mini accepted deployment source의 Darwin path와 일치하고 #117 launchd bootstrap의 system-only `PATH`를 완화하지 않는다. core는 exact runtime-config release의 artifact 이름 `compose.yaml`, project/config/image label, PostgreSQL running/healthy, project-scoped volume/internal network를 확인한다. PostgreSQL의 Compose label은 current `compose.yaml` 또는 같은 owner-only `runtime-config/releases` store에 있는 verified immutable prior release의 direct-child `compose.yaml`만 허용한다. current와 prior release는 각각 Legacy V1 또는 Manifested V2 exact allowlist, type/mode/owner, symlink·hardlink·private material 부재를 다시 검증하고 Compose bytes가 exact 같아야 한다. arbitrary external path, traversal, 다른 store, unverified/변조 release와 다른 Compose bytes는 fail closed한다. `pg_dump → dump fsync → pg_restore --list → Flyway post-check → sidecar/hash → partial directory fsync → final rename → backup directory fsync → last-success atomic replace → backup directory fsync` 순서로 publish한다. stdout에는 final artifact path, UTC timestamp와 schema version만 출력한다. failure는 nonzero exit다.
...
`launchd/com.homeserver.our-ledger-backup.plist.example`은 Mac mini local timezone에서 `00:35`, `06:35`, `12:35`, `18:35`에 `/Users/homeserver/Server/scripts/backup/backup-our-ledger-bootstrap.sh`를 argument 없이 호출한다. bootstrap은 exact current `backup-production.sh`에 `our-ledger-production`, app root `.env`, `/Users/homeserver/Server/backups/our-ledger/data`의 source-fixed authority만 전달한다. `launchd/com.homeserver.our-ledger-offsite.plist.example`은 committed backup window 뒤 `00:50`, `06:50`, `12:50`, `18:50`에 `/Users/homeserver/Server/scripts/offsite/offsite-our-ledger-bootstrap.sh run`만 호출한다. canonical source는 각각 `scripts/backup-our-ledger-bootstrap.sh`, `scripts/offsite-our-ledger-bootstrap.sh`이며 Manifested Runtime V2 payload로 전달한다. 두 source는 `/bin/bash`와 `/usr/bin/env -i`를 사용해 HOME/locale/system-only PATH만 남기고 exact `runtime-config/current` entrypoint를 실행한다. mutable checkout, private AGE identity, secret environment, retry와 fallback은 없다. plist나 fixed ingress를 설치·load/start하지 않았으며 source release·host update와 exact fixed-path install 뒤 실제 schedule activation은 #116의 별도 owner Gate다.
...
- 10C-2A source/drill, 10D-1 immutable transport, 10D-2A candidate migration, 10D-2B1 shared lock/state, 10D-2B2 deployment transaction, 10D-3A1/A2 bootstrap, 10D-3B3A encrypted offsite source, 10D-3B3C evolution bridge, 10D-3B3F V2 artifact와 10D-3B3J12 fixed schedule bootstrap source gate는 구현됐지만 후속 V2 release·publish·host update, production backup/migration/bootstrap/restore/offsite 실행과 fixed host ingress install·LaunchAgent schedule·보관·외부복제 활성화는 별도 운영 Gate다.
