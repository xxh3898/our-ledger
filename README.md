...
이 source gate는 `OUR_LEDGER_DEPLOY_ENABLED`가 없거나 `true`가 아니면 publish, Tailscale, SSH, Mac mini deploy를 전부 건너뛰고 검증만 수행한다. 10D-2A도 disposable PostgreSQL 계약만 검증하며 실제 GHCR package/credential, restricted host wrapper, operation lock, production status/backup/migration/HomeOps reporter, LaunchAgent, Cloudflare/Tunnel, production secret/User/DB와 deploy activation은 10D-2B/10D-3의 별도 승인 대상이다. Slice 10B PWA는 최종 한글 앱 이름과 production icon 확정 전까지 보류한다.
