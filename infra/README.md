...
이 gate는 helper/unit와 두 workflow source를 확인하고 amd64 host에서도 `scratch` Legacy V1 bridge runtime-config를 `--platform linux/arm64 --network none`으로 build/create/export해 exact regular-file allowlist와 `0600`/`0700` mode, expected directory hierarchy, label, forbidden material 부재, bridge script help와 Compose render를 검증한다. BuildKit이 자동 생성한 parent directory mode는 고정하지 않으며 실제 host release directory owner/mode는 B1/A2 source가 강제한다. 고유 development label의 image/container만 사용하고 cleanup 뒤 residue 0을 요구하며 registry login/push, Tailscale, SSH, production resource를 사용하지 않는다.

## Runtime-config evolution bridge

`./scripts/verify-runtime-config-evolution.sh`는 bridge Dockerfile의 actual linux/arm64 export를 production `9dd350b...`의 frozen manifest 없는 20-file V1 profile로 독립 검증한다. bridge 안의 기존 worker file은 V1 content hash/state를 그대로 읽고, root `runtime-manifest.json`이 있는 future V2 archive를 strict schema와 exact payload/mode/directory로 분류·추출·stage할 수 있다. V2 hash는 exact manifest bytes와 payload path/mode/content를 domain-separated하게 포함한다.

repository의 offsite source와 runtime change detector entry는 유지하지만 bridge image에는 manifest/offsite 두 file이 없다. `558f92e...` full runtime artifact의 direct production update, bridge release/publish/host update와 후속 V2 artifact 활성화는 모두 이 source gate 밖이다.
