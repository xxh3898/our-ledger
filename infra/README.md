...
Slice 10C의 immutable production origin, backup/restore, operational status/monitor source, Slice 10D-1의 immutable Release/Deploy source harness, Slice 10D-2A candidate migration gate, Slice 10D-2B1 host state/shared operation lock, Slice 10D-2B2 restricted deployment transaction, Slice 10D-3A1/A2 production Household/fresh-host bootstrap, Slice 10D-3B3A encrypted offsite source, Slice 10D-3B3C runtime-config evolution bridge와 Slice 10D-3B3F Manifested Runtime V2 source를 관리한다. 현재 구현은 image build, non-root Nginx, normal `production` API, profile-gated `api-migration`/`api-bootstrap`, `web`/`api`/`postgres` Compose, host 운영 source, reusable Full CI, default-off release workflow, frozen Legacy V1 호환 validator와 exact Manifested V2 source artifact, synthetic transaction/bootstrap/offsite harness까지다. 실제 V2 GHCR publish, Tailscale/SSH, Mac mini ingress/host install/deploy/status/backup/migration/bootstrap/offsite/restore/monitor/HomeOps reporter, forced-command 설치, actual bootstrap input/recipient/iCloud target, Cloudflare Tunnel/Access 설정, production secret/User/DB, LaunchAgent, retention 삭제·외부복제와 alert activation은 포함하지 않는다.
...
repository root의 `runtime-config.Dockerfile`은 root `runtime-manifest.json`과 manifest가 선언한 Compose/Nginx, local/offsite backup, fixed schedule bootstrap, status/monitor, host helper exact 24-file payload를 `scratch` Manifested V2 artifact로 패키징한다. fixed host state/lock/deployment/fresh-bootstrap worker와 test-only adapter는 `scripts/host_tools/`에 있다. restricted entrypoint는 `scripts/deploy-production.sh`, `scripts/bootstrap-production.sh`, status/monitor command는 `scripts/production-status.sh`, `scripts/monitor-production.sh`, offsite command는 `scripts/offsite-backup-production.sh`다. schedule fixed-ingress canonical source는 `scripts/backup-our-ledger-bootstrap.sh`, `scripts/offsite-our-ledger-bootstrap.sh`이며 실제 host path 설치와 LaunchAgent activation은 별도 owner Gate다. release contract와 runtime config detector는 `scripts/release_tools/`, `scripts/detect-runtime-config-change.sh`에 있다. host state, fixed bootstrap, runtime evolution, host/deploy/fresh-bootstrap transaction, restore, policy, offsite와 release 검증 진입점은 `scripts/verify-host-state.sh`, `scripts/verify-fixed-bootstrap.sh`, `scripts/verify-runtime-config-evolution.sh`, `scripts/verify-host-deploy-transaction.sh`, `scripts/verify-fresh-host-bootstrap.sh`, `scripts/verify-backup-restore.sh`, `scripts/verify-monitor-policy.sh`, `scripts/verify-offsite-backup.sh`, `scripts/verify-release-transport.sh`다. production Compose에 deploy/bootstrap/backup/offsite/status/monitor service나 host backup/input bind mount를 추가하지 않는다.
...
이 gate는 helper/unit와 두 workflow source를 확인하고 amd64 host에서도 `scratch` Manifested Runtime V2를 `--platform linux/arm64 --network none`으로 build/create/export한다. exact manifest bytes, 24 payload/7 directory, `0600`/`0700` mode, OCI identity, forbidden material 부재, local/offsite/fixed bootstrap script syntax와 Compose render를 bridge extractor/host validator로 검증한다. BuildKit이 자동 생성한 parent directory mode는 고정하지 않으며 실제 host release directory owner/mode는 B1/A2 source가 강제한다. 고유 development label의 image/container만 사용하고 cleanup 뒤 residue 0을 요구하며 registry login/push, Tailscale, SSH, production resource를 사용하지 않는다.

## Runtime-config evolution bridge와 Manifested V2 source

`./scripts/verify-runtime-config-evolution.sh`는 production `9dd350b...`의 frozen manifest 없는 20-file V1 profile/reference hash와 existing state 호환을 계속 독립 검증한다. 동시에 candidate Dockerfile의 actual linux/arm64 V2 export를 strict root manifest+24 payload/7 directory로 분류하고 bridge extractor→host validator→immutable stage/current→content hash re-read까지 수행한다. V2 hash는 exact manifest bytes와 payload path/mode/content를 domain-separated하게 포함한다.

V2 source artifact에는 offsite wrapper/worker가 포함되고 manifest, Dockerfile, runtime change detector가 같은 active source set을 따른다. `558f92e...` 과거 full runtime artifact의 direct production update는 계속 authority가 아니며 새 V2 release/publish/host update와 offsite 실행은 모두 이 source gate 밖이다.

## Host state/shared operation lock source

production worker root는 `/Users/homeserver/Server/apps/our-ledger` 상수로 고정되고 CLI/environment path override가 없다. `runtime-config/releases/<digesthex>`, `state/deployment.json`, `pending/transaction.json`, relative `current`와 `operations/lock`만 B1 managed authority다. directory/lock은 current owner mode `0700`, JSON은 `0600`이며 runtime release file은 frozen V1 또는 strict V2 profile의 `0600`/`0700`을 다시 검증한다.
...
`scripts/deploy-production.sh`와 `production_host deploy`에는 root, env, Compose, image, reporter, backup 또는 skip override가 없다. app root, project, env/backup/reporter path, image repository와 loopback target은 source에 고정된다. candidate image는 exact SHA tag, valid ID/repository digest, linux/arm64와 OCI source/revision/version을 모두 확인하며 runtime update는 exact digest export를 V1/V2로 먼저 분류한 뒤 profile의 allowlist/mode/type/size를 다시 검증한다. owner-only temporary Docker config와 runtime tree는 fixed `/private/tmp` 아래 app root와 disjoint하게 만들고 cleanup한다.
